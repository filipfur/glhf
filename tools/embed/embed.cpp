...
static std::string_view load_file(const char *path) {
    ...
    while ((n = fread(__membuf, 1, (sizeof __membuf) - 1, file_ptr)) != 0) {
        length += n;
    }
    fclose(file_ptr);
    __membuf[length] = '\0';
    return {__membuf, length};
}
...
void embed_to_cpp(const char *in_file, const char *out_file, const char *prefix,
                  const ReplaceList &replaceList) {
    auto content = load_file(in_file);

    const char *base_name = prefix ? prefix : "embed";
    ...
    fprintf(file, "// This file was generated by 'embed'\n#include <stddef.h>\n\n");

    fprintf(file, "extern const unsigned char _%s_%s[];\n", base_name, symbol_name.c_str());
    fprintf(file, "extern const size_t _%s_%s_length;\n", base_name, symbol_name.c_str());
    fprintf(file, "const unsigned char _%s_%s[] = {", base_name, symbol_name.c_str());

    // TODO: can probably be optimized with buffering?
    ...
    fprintf(file, " 0x00};\n");
    fprintf(file, "const size_t _%s_%s_length = %zu;\n", base_name, symbol_name.c_str(),
            content.length());

    fclose(file);
}

enum ParseMode { PARSE_NONE, PARSE_IN, PARSE_PREFIX, PARSE_REPLACE_A, PARSE_REPLACE_B, PARSE_OUT };
int main(int argc, char *argv[]) {
    ...
    embed_to_cpp(infile, outfile, prefix, replaceList);
    return 0;
}